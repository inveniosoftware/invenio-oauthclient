...
1. Edit your configuration and add:
   ...
   .. code-block:: python
       ...
       OAUTHCLIENT_REST_REMOTE_APPS = dict(
           cern_openid=cern_openid.REMOTE_REST_APP,
       )

       CERN_OPENID_CREDENTIALS = dict(
...
2. Register a new application with CERN OPENID visiting the page
   ...
   ``http://localhost:5000/api/oauth/authorized/cern_openid/``, if you have used
   the rest oauth application, or
...
OAUTHCLIENT_CERN_ALLOWED_ROLES = [
    'cern_user',
]
...
REMOTE_APP = dict(
    ...
    params=dict(
        ...
        app_key='CERN_OPENID_CREDENTIALS',
...
REMOTE_APP_RESOURCE_API_URL = 'https://auth.cern.ch/auth/realms/cern/protocol/openid-connect/userinfo'
REMOTE_APP_LOGOUT_URL = 'https://auth.cern.ch/auth/realms/cern/protocol/openid-connect/logout'
...
def account_roles_and_extra_data(account, resource,
    ...
    if cern_roles is None or not set(cern_roles).issubset(valid_roles):
        raise OAuthCERNRejectedAccountError(
            'User roles {0} are not one of {1}'.format(
                cern_roles, valid_roles), remote, resp, )
...
def on_identity_changed(sender, identity):
    ...
    client_id = current_app.config['CERN_OPENID_CREDENTIALS']['consumer_key']
    ...
    if account:
        ...
        roles.extend(
            account_roles_and_extra_data(account, resource,
