...
from helpers import check_redirect_location, check_response_redirect_url_args
...
from invenio_oauthclient.handlers.rest import authorized_signup_handler, \
    default_response_handler, disconnect_handler, oauth_error_handler, \
    response_handler_postmessage, signup_handler, oauth2_handle_error
...
def test_dummy_handler(remote_name, app_rest):
    """Test dummy handler."""

    signup_handler = app_rest.config['OAUTHCLIENT_REST_REMOTE_APPS'][
    ...
    app_rest.register_blueprint(blueprint_client)

    # Try to sign-up client
    app_rest.test_client().get(url_for('invenio_oauthclient.signup',
...
def test_response_handler(remote_name, app_rest):
    ...
    response = app_rest.test_client().get(
        url_for('invenio_oauthclient.signup', remote_app=remote_name)
    )
