...
def test_authorized_signup_username_already_exists(app, example_gitlab, user):
    """Test authorized callback with sign-up."""
    ...
    with app.test_client() as c:
        ...
        with mock.patch('github3.login') as MockLogin:
            ...
            assert resp.location == (
                'http://localhost' +
                url_for('invenio_oauthclient.signup', remote_app='gitlab')
            )
...
def test_authorized_reject(app):
    ...
    with app.test_client() as c:
        c.get(url_for('invenio_oauthclient.login', remote_app='gitlab'))
        resp = c.get(
            url_for('invenio_oauthclient.authorized',
...
def test_authorized_already_authenticated(app, models_fixture, example_gitlab):
    """Test authorized callback with sign-up."""
