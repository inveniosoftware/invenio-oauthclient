...
"""Test case for gitlab oauth remote app."""
...
from invenio_oauthclient.contrib.gitlab import authorized_rest
...
def test_login(app_rest):
    ...
    resp = client.get(
        url_for('invenio_oauthclient.rest_login', remote_app='gitlab',
                next='/someurl/')
    )
    ...
    assert params['scope'] == ['read_user email']
...
def test_authorized_signup_valid_user(app_rest, example_gitlab):
    ...
    with app_rest.test_client() as c:
        ...
        with mock.patch('requests.get') as MockLogin:
            ...
            mock_response(app_rest.extensions['oauthlib.client'], 'gitlab',
                          example_gitlab)
    ...
    with app_rest.test_client() as c:
        ...
        with mock.patch('requests.get') as MockLogin:
            ...
            assert resp.location == (
                'http://localhost' +
                url_for('invenio_oauthclient.rest_signup', remote_app='gitlab')
            )
...
def test_authorized_reject(app_rest):
    ...
    with app_rest.test_client() as c:
        c.get(url_for('invenio_oauthclient.rest_login', remote_app='gitlab'))
        resp = c.get(
            url_for('invenio_oauthclient.rest_authorized',
                    remote_app='gitlab', error='access_denied',
...
def test_authorized_rest_handler(app_rest):
    ...
    remote = oauth.remote_apps['gitlab']
    # General error
    ...
    resp = authorized_rest(example_response, remote)
    check_redirect_location(resp, '/oauth/login/gitlab/')
