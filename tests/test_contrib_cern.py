...
def test_fetch_groups(app, example_cern):
    ...
    app.config["OAUTHCLIENT_CERN_HIDDEN_GROUPS_RE"] = (re.compile(r"Group[1-3]"),)
...
def test_account_info(app, example_cern):
    ...
    assert account_info(ioc.remote_apps["cern"], {}) == dict(
        user=dict(
            email="test.account@cern.ch",
            profile={"full_name": "Test Account", "username": "taccount"},
        ),
        external_id="123456",
...
def test_login(app):
    ...
    resp = client.get(
        url_for("invenio_oauthclient.login", remote_app="cern", next="/someurl/")
    )
    assert resp.status_code == 302
...
def test_account_info_not_allowed_account(app, example_cern):
    ...
    app.config["OAUTHCLIENT_CERN_ALLOWED_IDENTITY_CLASSES"] = ["another cern type"]
    ioc = app.extensions["oauthlib.client"]
