...
def test_login(app_rest):
    ...
    resp = client.get(
        url_for("invenio_oauthclient.rest_login", remote_app="github", next="/someurl/")
    )
...
class MockGh(object):
    ...
    def emails(self):
        ...
        yield Email(verified=True, primary=True, email=self._email)
...
def test_authorized_signup_valid_user(app_rest, example_github):
    ...
    with app_rest.test_client() as c:
        ...
        with mock.patch("github3.login") as MockLogin:
            ...
            # request)
            resp = c.get(url_for("invenio_oauthclient.rest_login", remote_app="github"))

            assert resp.status_code == 302

            mock_response(
                app_rest.extensions["oauthlib.client"], "github", example_github
            )
            ...
            resp = c.get(
                url_for("invenio_oauthclient.rest_disconnect", remote_app="github")
            )
...
def test_authorized_signup_username_already_exists(app_rest, example_github, user_rest):
    """Test authorized callback with sign-up."""
    ...
    with app_rest.test_client() as c:
        ...
        with mock.patch("github3.login") as MockLogin:
            ...
            assert (
                1
                == UserIdentity.query.filter_by(
                    method="github", id_user=my_user.id, id="githubuser"
                ).count()
            )
            ...
            assert RemoteAccount.query.filter_by(user_id=my_user.id).count() == 0
            assert RemoteToken.query.count() == 0
...
def test_authorized_already_authenticated(app_rest, models_fixture, example_github):
    """Test authorized callback with sign-up."""
    ...
    with mock.patch("github3.login") as MockLogin:
        ...
        with app_rest.test_client() as client:
            ...
            # Ensure remote apps have been loaded (due to before first
            # request)
            client.get(url_for("invenio_oauthclient.rest_login", remote_app="github"))
            ...
            assert (
                0
                == UserIdentity.query.filter_by(
                    method="github", id_user=u.id, id="githubuser"
                ).count()
            )
